...
export function EventoEditar() {
  ...
  useEffect(() => {
    ...
    if (foundEvent) {
      setFecha(foundEvent.fecha);
      setHoraEvento(foundEvent.horaEvento);
      setAnio(foundEvent.anio);
      setMes(foundEvent.mes);
      setMes_1(foundEvent.mes_1);
      setSem(foundEvent.sem);
      setDia(foundEvent.dia);
      setRangoHorario(foundEvent.rangoHorario);
      setTipoIncidente(foundEvent.tipoIncidente);
      setDescripcion(foundEvent.descripcion);
      setUbicacion(foundEvent.ubicacion);
      setTipoVia(foundEvent.tipoVia);
      setDireccionVia(foundEvent.direccionVia || "");
      setLugarIncidente(foundEvent.lugarIncidente);
      setModeloMR(foundEvent.modeloMR || "");
      setNroMR(foundEvent.nroMR || "");
      setNroCarrera(foundEvent.nroCarrera || "");
      setPersonalInvolucrado(foundEvent.personalInvolucrado || "");
      setTipoCausa(foundEvent.tipoCausa);
      setPosibleCausa(foundEvent.posibleCausa || "");
      setInformacionAdicional(foundEvent.informacionAdicional || "");
      setCamaraMonitoreada(foundEvent.camaraMonitoreada);
      setCamaraSeleccionada(foundEvent.camaraSeleccionada || "");
      setDemora(foundEvent.demora || "");
    }
      ...
      <MonitoreoShell>
        <div className="max-w-2xl mx-auto">
          <Card className="p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-ink mb-2">Evento actualizado correctamente</h2>
            <p className="text-ink-quiet mb-6">El evento ha sido actualizado exitosamente en el sistema</p>

            <div className="flex items-center justify-center gap-3">
              <Button onClick={() => navigate(`/monitoreo/eventos/${id}`)}>
                Ver evento
              </Button>
              <Button variant="outline" onClick={() => navigate("/monitoreo")}>
                Volver al dashboard
              </Button>
            </div>
          </Card>
    ...
    <MonitoreoShell>
      <div className="w-full px-4">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-[22px] font-bold text-ink tracking-tight">Editar Evento</h1>
            <p className="text-[13px] text-ink-quiet mt-1">
              Modifique la información del evento. Los campos marcados con * son obligatorios.
            </p>
          </div>
        </div>
